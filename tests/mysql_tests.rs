//! Integration tests for MySQL-based distributed locks.
...
/// Helper to get MySQL connection string from environment or use default.
...
async fn test_exclusive_lock_acquisition() {
    ...
    let provider = MySqlLockProvider::new(url).await.unwrap();
...
async fn test_blocking_acquire() {
    ...
    let acquire_task = tokio::spawn(async move {
        let provider2 = MySqlLockProvider::new(url_clone).await.unwrap();
