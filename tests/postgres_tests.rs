...
use distributed_lock_core::traits::{DistributedLock, DistributedReaderWriterLock};
...
async fn test_reader_writer_lock_semantics() {
    ...
    assert!(write_handle.is_none(), "Write lock should fail when readers hold lock");
    ...
    assert!(write_handle2.is_some(), "Write lock should succeed after readers release");

    // Test 4: Readers block when writer holds lock
    let read_handle4 = rw_lock.try_acquire_read().await.unwrap();
    assert!(read_handle4.is_none(), "Read lock should fail when writer holds lock");
    
    let write_handle3 = rw_lock.try_acquire_write().await.unwrap();
    assert!(write_handle3.is_none(), "Another write lock should fail when writer holds lock");
    ...
    assert!(read_handle5.is_some(), "Read lock should succeed after writer releases");
...
async fn test_reader_writer_lock_blocking_acquire() {
    ...
    let rw_lock_clone = &rw_lock;
    let acquire_task = tokio::spawn(async move {
        rw_lock_clone.acquire_read(Some(Duration::from_millis(200))).await
    });
    ...
    let result = timeout(Duration::from_secs(1), acquire_task)
        ...
        .unwrap()
        .unwrap();
    assert!(result.is_ok(), "Waiting task should acquire read lock after writer releases");
    result.unwrap().release().await.unwrap();
}
