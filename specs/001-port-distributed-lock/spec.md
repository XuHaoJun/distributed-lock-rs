...
- Q: What is the minimum supported Rust version (MSRV)? → A: MSRV 1.88 - enables native async traits
...
- Rust's async ecosystem (tokio or async-std) is used for async operations; tokio is the primary target runtime. Minimum supported Rust version (MSRV) is 1.88, enabling native async traits without the async-trait crate workaround.
